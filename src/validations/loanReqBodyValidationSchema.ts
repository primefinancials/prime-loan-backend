...
export const createClientLoanSchema = Joi.object({
  ...
  nin: Joi.string().required().label("National Identification Number"),
  ...
  annual_income: Joi.number().positive().required().label("Annual Income"),
  ...
  amount: Joi.number().positive().required().label("Loan Amount"),
  reason: Joi.string().required().label("Loan Reason"),
  base64Image: Joi.string().optional().label("Image"),
  outstanding: Joi.number().optional().label("Outstanding Amount"),
  ...
  duration: Joi.number().required().positive().label("Duration"),
  repayment_amount: Joi.number().required().positive().label("Repayment Amount"),
  percentage: Joi.number().required().positive().label("Percentage"),
  loan_date: Joi.date().required().label("Loan Date"),
  repayment_date: Joi.date().required().label("Repayment Date"),
